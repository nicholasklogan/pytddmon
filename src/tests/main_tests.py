...
def test_kata_gen(input_kata_name, expected_kata_filename, expected_kata_content):
    # Exercise & Verify
    assert pytddmon2.main(kata_handler=lambda kata_filename, kata_content: kata_filename==expected_kata_filename and kata_content == expected_kata_content,
                          cmd_args=tuple(), kata_name=input_kata_name)
